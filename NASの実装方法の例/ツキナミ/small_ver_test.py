...
fine_tuned_model = AutoModelForSequenceClassification.from_pretrained("./result/novel_classifier_prototype_final(small_ver)")
...
headers = ["Sample Text (初5字)", "True Label", "Fine-tuned Correct?", "Pre-trained Correct?"]
...
print("\nTotal Scores:")
total_scores = [
    ["Fine-tuned", f"{ft_correct_count}/{len(test_samples)}"],
    ["Pre-trained", f"{pt_correct_count}/{len(test_samples)}"]
]
print(tabulate(total_scores, headers=["Model", "Correct Predictions"], tablefmt="grid"))
